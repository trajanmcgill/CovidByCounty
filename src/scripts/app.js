...
let appLogic = (function()
{
	...
	function parseDate(dateString)
	{
		let pieces = dateString.split("/"),
			parsedDate = new Date(pieces[2], pieces[0], pieces[1]);
		return parsedDate;
	} // end parseDate()
...
let app = new Vue(
	{
		...
		mounted: function()
		{
						...
						function()
						{
							...
							fetch("data/countyPopulations2018.csv")
								...
								.then(
									text =>
									{
										appLogic.storeCountyCaseData(text, vueObject.allCountyData);
				
										// CHANGE CODE: coloration test
										window.allCountyData = vueObject.allCountyData;
										window.sequence = view.buildDataAnimation(
											vueObject.allCountyData,
											view.FactType.Cases,
											view.DataViewType.InstantaneousValue,
			...
			view.whenDocumentLoaded(runSetup);
		}
	});
